export function initSearch(CodeMirror) {
    'use strict'

    function searchOverlay(query, caseInsensitive, flags) {
        if (typeof query == 'string') query = new RegExp(query.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&'), caseInsensitive ? 'gi' : 'g')
        else query = new RegExp(query.source, flags)

        console.log('Query: ', query, query.lastIndex)
    ...
    function startSearch(cm, state, query, flags='ig') {
        state.queryText = query
        state.query = parseQuery(query)
        cm.removeOverlay(state.overlay, queryCaseInsensitive(state.query))
        state.overlay = searchOverlay(state.query, queryCaseInsensitive(state.query), flags)
        ...
        if (persistent && cm.openDialog) {
            ...
            var searchNext = function(query, event) {
                ...
                findNext(cm, event.shiftKey, function(_, to) {
                    var dialog
                    if (to.line < 3 && document.querySelector && (dialog = cm.display.wrapper.querySelector('.CodeMirror-dialog')) && dialog.getBoundingClientRect().bottom - 4 > cm.cursorCoords(to, 'window').top) (hiding = dialog).style.opacity = 0.4
                })
            }
    ...
    CodeMirror.commands.startSearch = startSearch
